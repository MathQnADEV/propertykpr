    ...
    <aside id="agentSidebar" class="agent-sidebar fixed top-0 left-0 h-full w-[270px] bg-[#060922] text-white z-50 lg:translate-x-0 flex flex-col">
        <div class="flex-shrink-0 px-6 pt-8 pb-6">
            ...
            <div class="mt-4 flex items-center gap-3 p-3 rounded-2xl bg-white/5">
                <div class="w-10 h-10 rounded-full bg-white flex items-center justify-center text-[#111111] font-bold text-sm">
        ...
        <nav class="flex-1 overflow-y-auto custom-scroll px-4 pb-6">
            ...
            <ul class="flex flex-col gap-1 mb-6">
                ...
                <li>
                    <a href="{{ route('agent.notifications') }}" class="nav-item relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 {{ request()->routeIs('agent.notifications*') ? 'active bg-white/10 text-white font-semibold' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                        <span class="text-sm">Notifikasi</span>
                        @php $unreadCount = App\Models\SystemNotification::where("user_id", Auth::id())->where("is_read", false)->whereIn("type", ["broadcast", "direct"])->count(); @endphp
                        @if($unreadCount > 0)<span class="ml-auto bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{{ $unreadCount }}</span>@endif
                    </a>
                </li>
                <li>
                    <a href="{{ route('agent.property-browse') }}" class="nav-item relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 {{ request()->routeIs('agent.property-browse*') ? 'active bg-white/10 text-white font-semibold' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <span class="text-sm">Lihat Property</span>
                    </a>
                </li>
                <li>
                    <a href="{{ route('agent.payments') }}" class="nav-item relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 {{ request()->routeIs('agent.payments*') ? 'active bg-white/10 text-white font-semibold' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
            ...
            <ul class="flex flex-col gap-1">
                ...
                <li>
                    <a href="{{ route('agent.activity-log') }}" class="nav-item relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 {{ request()->routeIs('agent.activity-log*') ? 'active bg-white/10 text-white font-semibold' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                ...
                <li>
                    <a href="{{ route('front.index') }}" class="nav-item relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 hover:bg-white/5 hover:text-white transition-all duration-200">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9"/></svg>
                ...
                <li>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="nav-item w-full relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 hover:bg-white/10 hover:text-white transition-all duration-200">
        ...
        <header class="sticky top-0 z-30 bg-[#F8F8FA]/80 backdrop-blur-xl px-4 lg:px-8 py-4">
            <div class="flex items-center justify-between bg-white rounded-2xl px-4 lg:px-6 py-3 shadow-sm">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="w-10 h-10 rounded-full bg-[#060922] flex items-center justify-center text-white font-bold text-sm">
                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                    </div>
                </div>
            </div>
