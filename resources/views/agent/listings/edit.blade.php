...
@section('content')
    ...
    @if($errors->any())
        <div class="bg-[#444444]/5 border border-[#444444]/20 rounded-2xl p-4 mb-6 flex items-start gap-3">
            <svg class="w-5 h-5 text-[#444444] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
            <div>
                <p class="text-sm font-semibold text-[#444444] mb-1">Ada {{ $errors->count() }} kesalahan yang perlu diperbaiki:</p>
                <ul class="text-sm text-[#444444]/80 space-y-0.5 list-disc list-inside">
    ...
    @php
        $inputBase = 'w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-[#111111]/20 focus:border-[#111111] transition-all';
        $inputOk  = 'border-[#F2F2F4]';
        $inputErr = 'border-[#444444] bg-[#444444]/5';
    @endphp
    ...
    <form method="POST" action="{{ route('agent.listings.update', $house) }}" enctype="multipart/form-data" id="editListingForm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl p-5 border border-[#F2F2F4]">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            @error('name')
                                <p class="text-xs text-[#444444] mt-1.5 flex items-center gap-1">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            ...
                            {{-- Luas Tanah --}}
                            <div>
                                <label class="block text-sm font-semibold text-[#060922] mb-1.5">Luas Tanah <span class="text-[#444444]">*</span></label>
                            ...
                            {{-- Luas Bangunan --}}
                            <div>
                                <label class="block text-sm font-semibold text-[#060922] mb-1.5">Luas Bangunan <span class="text-[#444444]">*</span></label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            {{-- Daya Listrik --}}
                            <div>
                                <label class="block text-sm font-semibold text-[#060922] mb-1.5">Daya Listrik (W) <span class="text-[#444444]">*</span></label>
                            ...
                            {{-- Status Ketersediaan --}}
                            <div>
                                <label class="block text-sm font-semibold text-[#060922] mb-1.5">Status <span class="text-[#444444]">*</span></label>
                        ...
                        @foreach($facilities as $facility)
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-[#F2F2F4] cursor-pointer hover:border-[#111111]/30 has-[:checked]:border-[#111111] has-[:checked]:bg-[#111111]/5 transition-all">
                                <input type="checkbox" name="facilities[]" value="{{ $facility->id }}"
                                    class="w-4 h-4 rounded border-gray-300 text-[#111111] focus:ring-[#111111]"
            ...
            <div class="space-y-6">
                ...
                {{-- Thumbnail --}}
                <div class="bg-white rounded-2xl p-5 border {{ $errors->has('thumbnail') ? 'border-[#444444]' : 'border-[#F2F2F4]' }}">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="font-bold text-[#060922]">Thumbnail <span class="text-[#444444]">*</span></h2>
                        @if($house->thumbnail)
                            <button type="button" id="removeThumbnailBtn" onclick="removeThumbnail()"
                                class="text-xs font-semibold text-[#444444] hover:underline">Hapus</button>
                        @endif
                    </div>
                    <input type="hidden" name="remove_thumbnail" id="removeThumbnailInput" value="0" />
                    <div id="thumbnailPreview"
                        class="relative w-full h-40 rounded-xl border-2 border-dashed {{ $errors->has('thumbnail') ? 'border-[#444444]/50' : 'border-[#F2F2F4]' }} flex items-center justify-center cursor-pointer hover:border-[#111111]/30 transition-colors overflow-hidden"
                    ...
                    @error('thumbnail')
                        <p class="text-xs text-[#444444] mt-2 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                            {{ $message }}
                        </p>
                    @enderror
                    {{-- Client-side warning (shown before submit) --}}
                    <p id="thumbnailWarning" class="text-xs text-[#444444] mt-2 hidden flex items-center gap-1">
                        <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                ...
                <div class="bg-white rounded-2xl p-5 border border-[#F2F2F4]">
                            ...
                            @foreach($house->photos as $photo)
                                <div class="relative w-full h-24 rounded-xl overflow-hidden group" id="photo-wrap-{{ $photo->id }}">
                                    ...
                                    <button type="button" onclick="removePhoto({{ $photo->id }})"
                                        class="absolute top-1 right-1 w-6 h-6 bg-[#444444] rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    @endif
                    @error('photos.*')
                        <p class="text-xs text-[#444444] mb-2 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                            {{ $message }}
                        </p>
                    @enderror
